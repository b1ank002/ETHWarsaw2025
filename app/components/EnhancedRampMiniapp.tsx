...
export default function EnhancedRampMiniapp({ className = "" }: EnhancedRampMiniappProps) {
  ...
  const fetchCeloPrice = useCallback(async () => {
    try {
      ...
      tempSdk.on("*", (event) => {
        console.log("EnhancedRampMiniapp - Ramp pricing event:", event.type, event.payload);
          ...
          if (payload?.purchase) {
            ...
            if (purchase.cryptoAmount && purchase.fiatValue) {
              ...
              if (cryptoAmount > 0 && fiatAmount > 0) {
                ...
                setCeloPrice(price);
                console.log(`EnhancedRampMiniapp - Updated CELO price from Ramp SDK: ${price.toFixed(4)} PLN per CELO`);
                priceCaptured = true;
      ...
      const timeoutId = setTimeout(() => {
        if (!priceCaptured) {
          console.log("EnhancedRampMiniapp - No price captured from Ramp SDK, using fallback");
        ...
        try {
          ...
          console.log("EnhancedRampMiniapp - Error destroying temp Ramp SDK:", error);
        }
      }, 8000);

    } catch (error) {
      console.error('EnhancedRampMiniapp - Error fetching CELO price from Ramp SDK:', error);
            ...
            <div className="mb-6">
              ...
              <div className="bg-green-50 border-2 border-green-300 rounded-xl p-6 text-center">
                ...
                <div className="text-xs text-green-500 mt-1">
                  Ramp Network pricing • Updates every 15s
                </div>
              </div>
            </div>
